...
def get_custom_bandwidth(low, high, url):
    relays = range_relays(controller, low, high)
    ...
    while (loopContinue):
        ...
        # print entry_nodes
        # print random.randint(0, len(entry_nodes) -1)
...
def main():
    ...
    # websites = ["baidu.com", "labanquepostale.fr", "credit-agricole.fr", "folha.uol.com.br"] # Don't Write (https://www.)

    # websites = ["google.com.au"]

    # numberRelays = 10
    url = "cachefly.cachefly.net/100mb.test"
    # experiment_tor(websites, 2)
    # experiment_smartor(websites, 2, 5)
    # experiment_smartor_bandwidth(websites, 2, 5)
    experiment_bandwidth_test(url, 1, 10, 20)

def experiment_bandwidth_test(url, ntimes, low, high):
    
    circuit = get_custom_bandwidth(low, high, url)
    url = 'https://' + url
